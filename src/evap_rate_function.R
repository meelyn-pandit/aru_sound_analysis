...
# Calculate volume of water evaporated (mL) from a 10cm radius area --------

evap_volume = function(x){
  #' Calculate volume of water evaporated (mL) from a 10cm radius area, final units are mL/cm2/day
  #'@param x = evaporation rate (mm/day)
  #'@returns evap_vol (mL/cm^2/day)
  evap_vol = (x*(3.14*10000)) # evaporation rate is the height of the cylinder, convert 10cm to 100mm radius, returns evaporation rate in mm^3
  evap_vol = evap_vol/1000 # convert from mm^3 to mL
  evap_vol = evap_vol/(3.14*100) # divide by surface area of 10cm pan to get mL/cm^2/day
  return(evap_vol)
}
