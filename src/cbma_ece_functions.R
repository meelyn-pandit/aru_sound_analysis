cbma_water_climate = function(pc){
  m1_cbmawl = lm(pc ~ ws_site*water + mas_bin + date, 
                  data = cbmawl_clmas)
  summary = summary(m1_cbmawl)
  diagnostics = assump(m1_cbmawl)
  emm = emtrends(m1_cbmawl, ~ ws_site*water|mas_bin, var = "water", type = 'response',weights = "cells")
  ...
  cntrst_summary = contrast(emm,
                            method = list("Site1:Closed - Site2:Open" = ws1w0-ws2w1))
...
cbma_water_impact = function(data,
                             pc){
  im1_cbmawl = lm(pc ~ ws_site*water + mas_bin + scale(date), 
                   data = data)
  ...
  cntrst_summary = contrast(emm,
                  method = list("Site1:Open - Site2:Open" = ws1w1-ws2w1,"Site1:Closed - Site2:Open" = ws1w0-ws2w1))
...
cbma_ece_table = function(climate_pc1_table,
  ...
  cbma_ece_combined = cbind(table1, table2, table3) %>%
    gt(.) %>%
    cols_align('center') %>%
    tab_spanner(
      label = md("**PC1 - Acoustic Diversity**"),
