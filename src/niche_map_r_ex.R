...
# Run calculation with all default values, specifying a local air  --------
...
# Metabolic chamber example: Australian budgerigar-----------------------------------------------
...
# Extract Results - Metabolic Chamber Example - Budgrigar -----------------
...
# Microclimate model integration example - Australian Budriegar -----------
...
dates <- micro$dates
dates_utc <- as_datetime(micro$dates, tz = "UTC");dates_utc
...
legend(x = 1, y = 65, legend = c("core", "skin dorsal", "skin ventral",
       ...
       ncol = 2)
plot((H2O/(AMASS * 1000)) * 100 ~ dates_utc, type = "l", ylab = "H2O loss, % body mass / h",
     xlab = "time", ylim = c(0, 6))
