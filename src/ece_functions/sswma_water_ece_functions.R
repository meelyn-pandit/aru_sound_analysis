sswma_water_climate = function(data,
                               yvar,
                               xvar){ # do not separate by mas_bins, not enough for statistical power
  m1_sswmawl = lm(yvar ~ ws_site*water*xvar+mas_bin + scale(date), 
                  data = data)
  summary = summary(m1_sswmawl)
  diagnostics = assump(m1_sswmawl)
  emm = emtrends(m1_sswmawl, ~ ws_site*water, var = "xvar", type = 'response',weights = "cells")
  ...
  cntrst_summary = contrast(emm, 
           method = list("Site2:Open - Site1:Closed" = ws2w1 - ws1w0, # not comparing ws1w1 because there's not enough observations of them
                         "Site2:Open - Site3:Closed" = ws2w1 - ws3w0),adjust = "bonferroni")
...
sswma_water_impact = function(data,
                              yvar,
                              xvar){
  im1_sswmawl = lm(yvar ~ ws_site*water*xvar + mas_bin + scale(date), 
                   data = data)
  summary = summary(im1_sswmawl)
  emm = emtrends(im1_sswmawl, ~ ws_site*water, var = "xvar", type = 'response',weights = "cells")
