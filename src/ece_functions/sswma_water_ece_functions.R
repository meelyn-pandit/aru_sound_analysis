sswma_water_climate = function(pc){ # do not separate by mas_bins, not enough for statistical power
  m1_sswmawl = lm(pc ~ ws_site*water*gh+mas_bin + date, 
                  data = sswmawl_clmas)
  summary = summary(m1_sswmawl)
  diagnostics = assump(m1_sswmawl)
  emm = emtrends(m1_sswmawl, ~ ws_site*water, var = "gh", type = 'response',weights = "cells")
...
sswma_water_impact = function(data,
                              pc){
  im1_sswmawl = lm(pc ~ ws_site*water*gh + mas_bin + scale(date), 
                   data = data)
  summary = summary(im1_sswmawl)
  emm = emtrends(im1_sswmawl, ~ ws_site*water, var = "gh", type = 'response',weights = "cells")
