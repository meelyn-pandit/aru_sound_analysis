...
ABMout <- apply(terr_freq_ewl_site, MARGIN = 1, function(x){
  aridityABM(HexSize=x[1], #territory size in m, original territory size is 1000m
           ...
           plot = F,
           site_lab = "lwma") #plot the grid and hexagon territories
})
...
ABMout_final = #Run the Aridity ABM
  # library(pbapply)
...
save(ABMout_final, file = "data_clean/abm_results/lwma_abmout.Rdata") #change file name based on which weather conditons you use and if you the EWL equation is included or not.
