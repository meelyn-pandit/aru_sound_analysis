...
find_gam_ip = function(gam,
  ...
  plot(fd, sizer = TRUE)
  CI <- lapply(fd["ew_vol"], function(x) t(apply(x$simulations, 1, quantile, probs = c(0.025, 0.975))))
  first.zero.slope.index <- min(which(sign(CI$ew_vol[, "2.5%"]) != sign(CI$ew_vol[, "97.5%"])))
  ...
  gam_der = as.data.frame(cbind(fd$eval,CI$ew_vol))
  gam_der2 = gam_der %>%
    dplyr::filter(ew_vol > ew_vol_limit)
  impact = gam_der2$ew_vol[min(which(sign(gam_der2[, "2.5%"]) != sign(gam_der2[, "97.5%"])))]
  return(impact)
}
