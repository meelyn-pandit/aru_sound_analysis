# Run analysis on synthetic budgie dataset with 20-100 unique elements
...
warbleR_options(wl = 300, parallel = 20, ovlp = 90, pb = FALSE, bp = "frange", dens = 0.7, img = FALSE)


# load extended selection tables with synthetic calls. This file is big.set
sim.ests <- readRDS(file.path(getwd(), "Synth budgie large repertoire 24 ESTs.RDS") )
...
for (i in 1:length(sim.ests)){
  ...
  sil_width <- c(NA)
  sil_max<-true_rep_size + 25
  sil_min<-true_rep_size - 25
  sil_min<-max(sil_min,5)
  ...
  plot_list[[i]] <-  ggplot(aes(x = X, y = Y), data = tsne_data) +
    ...
    ggtitle(sprintf("BUDGIE large syth dataset %i\n%s\nnum clusts = %i", i,names(sim.ests[i]),num_clusts)) +
  ...
  # Accuracy
  acc <- corr.assnd / nrow(acous.parms)
  accuracy_all[i]<-acc
...
saveRDS(datalist, "large.budgie.results.RDS")
