# Run analysis on synthetic LBH dataset with 5-15 unique elements
...
sim.ests <- readRDS(file.path(getwd(), "Synth LBH small repertoire 24 ESTs.RDS") )
...
for (i in 1:length(sim.ests)){
  ...
  plot_list[[i]] <-  ggplot(aes(x = X, y = Y), data = tsne_data) +
    ...
    ggtitle(sprintf("LBH small syth dataset %i\n%s\nnum clusts = %i", i,names(sim.ests[i]),num_clusts)) +
...
saveRDS(datalist, "small.lbh.results.RDS")
