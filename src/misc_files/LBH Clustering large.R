# Run analysis on synthetic lbh dataset with 20-100 unique elements
...
sim.ests <- readRDS(file.path(getwd(), "Synth LBH large repertoire 24 ESTs.RDS") )
...
for (i in 1:length(sim.ests)){
  ...
  plot_list[[i]] <-  ggplot(aes(x = X, y = Y), data = tsne_data) +
    ...
    ggtitle(sprintf("LBH large syth dataset %i\n%s\nnum clusts = %i", i,names(sim.ests[i]),num_clusts)) +
...
saveRDS(datalist, "large.lbh.results.RDS")
